...
/  date:       March, 2010
/              Write the output to a unique file
/  modified:   Dan Reynolds, July 2010, added case-B recombination rates
/
/  PURPOSE:
...
extern "C" void FORTRAN_NAME(calc_rates)(
     ...
     float *f3, int *iradtype, int *casebrates,
...
int InitializeRateData(FLOAT Time)
{
  ...
  CoolData.f0to3                    = 0.1;
  CoolData.RadiationRedshiftOn      = 11.0;
  CoolData.RadiationRedshiftOff     = 0.0;
  CoolData.RadiationRedshiftFullOn  = 10.0;
  CoolData.RadiationRedshiftDropOff = 2.0;
  ...
  FORTRAN_NAME(calc_rates)(
     ...
     &RadiationFieldType, &RateData.CaseBRecombination,
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    FILE *outfptr;
    if ((outfptr = fopen("CoolData.log", "a")) == NULL) {
      fprintf(stderr,"Error opening CoolData parameter output file.\n");
      return FAIL;
    }
    ...
    fprintf(outfptr,"RateDataCaseBRecombination = %"ISYM"\n",
	    RateData.CaseBRecombination);
