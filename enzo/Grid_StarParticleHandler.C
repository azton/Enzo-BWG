...
/* Simpson 2015 kinetic feedback maker */
...
/* Putting in Simpson 2015 kinetic feedback model */
...
/* mechanical feedback modeled on FIRE-2 (Hopkins et al., 2017) */
extern "C" void FORTRAN_NAME(star_feedback_mechanical)(int *nx, int *ny, int *nz, 
                           ...
                           int *imulti_metals, hydro_method *imethod,
                           float *dt, float *r, float *dx, 
                           float *t, float *z, float *h, float *omega_m,
                           float *omega_l, float *d1, 
                           float *x1, float *v1,float *t1, int *nmax,
                           FLOAT *xstart, FLOAT *ystart, FLOAT *zstart,
                           int *ibuff, FLOAT *xp, FLOAT *yp, 
    ...
    if (StarParticleCreation == 3) {
      ...
      FORTRAN_NAME(star_maker3)(
       ...
       tg->ParticleVelocity[0], tg->ParticleVelocity[1],
          tg->ParticleVelocity[2],
       tg->ParticleMass, tg->ParticleAttribute[1], tg->ParticleAttribute[0],
          tg->ParticleAttribute[2], &StarMakerTypeIaSNe, BaryonField[MetalIaNum],
          &metalfSNIa);
    }
...
/* adding distributed starmaker and feedback */
    ...
    if (StarParticleCreation == 15 && level >= StarMakeLevel){
      int StarMakerTypeIaSNe = 0; // SN type Ia metallicity flag
      float metalfSNIa = 0.0; // metallicity fraction of particle from Ia
      int MetalIaNum = -1; // number of the (nonexistant) SNIa metal field
// #define USE_C_MAKER
