...
void CommunicationBroadcastValue(int *Value, int BroadcastProcessor);
...
*/
	...
	if ((fptr = fopen("subcycleCount", "r")) == NULL) {
      ...
      if( outputNow != -1 )
	if (unlink("outputNow")) {
	  printf("Error deleting 'outputNow'");
      return FAIL;
	}
      if( subcycleCount != -1 )
	if (unlink("subcycleCount")) {
      ...
      if( stopNow != -1 )
	if (unlink("stopNow")) {
	  printf("Error deleting stopNow");
      return FAIL;
	} 
      if( checkpointDumpNow != -1 )
	if (unlink("checkpointDump")) {
        printf("Error deleting checkpointDump");
        return FAIL;
	} 
    ...
    CommunicationBroadcastValue(OutputFlagArray, ROOT_PROCESSOR);
...
#endif
			   LevelArray[level]->GridData->ReturnTime()) == FAIL) {
            printf("Error in Group_WriteAllData.");
            return FAIL;
    }
// #else
