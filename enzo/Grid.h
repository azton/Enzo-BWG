...
/* Count active grids on a level */

   int CountGridsOnLevel(int level);
...
//
   ...
   float* AccessEmissivityField0();
   float* AccessEmissivityField1();
   float* AccessKPhHI();
...
/* PPM Direct Euler Solver - WOC version */
...
/* PPM Direct Euler Solver - John Wise Version */
...
/* added to import solvers from enzo-dev */
int ComputePressure(FLOAT time, float *pressure,
                          float MinimumSupportEnergyCoefficient,
                          int IncludeCRs=0);
...
/* Dual energy cosmological ionization test problem: initialize grid (SUCCESS or FAIL) */
  int DualCosmoIonizationInitializeGrid(int NumChem, float V0Const, float V1Const, 
					float V2Const, float IEConst, float XrConst,
					float UVConst,  float HMassFrac, float InitFracHII, 
					float InitFracHeII, float InitFracHeIII, 
					float OmegaBaryonNow, int local);

/* Dual energy Rad-Hydro ionization test problem: initialize grid (SUCCESS or FAIL) */
  int DualRHIonizationTestInitializeGrid(int NumChem, float DensityConst, 
					 float V0Const, float V1Const, 
					 float V2Const, float IEConst, 
					 float UVConst, float XrConst, 
					 float HMassFrac, float InitFracHII, 
					 float InitFracHeII, float InitFracHeIII, 
					 int local);

/* Dual energy Rad-Hydro ionization test problem: initialize grid (SUCCESS or FAIL) */
  int DualRadConstTestInitializeGrid(int NumChem, float DensityConst, 
				     float V0Const, float V1Const, 
				     float V2Const, float IEConst, 
				     float UVConst, float XrConst, 
				     float HMassFrac, float InitFracHII, 
...
/* Dual energy streaming radiation test problem: initialize grid (SUCCESS or FAIL) */
  int DualRadStreamTestInitializeGrid(int NumChem, float Density, float Vx, 
				      float Vy, float Vz, float IE, float UV, 
				      float Xr, float HMassFrac, float InitFracHII, 
				      float InitFracHeII, float InitFracHeIII, 
				      int StreamDim, int StreamDir, int StreamType, 
				      int local);
...
/* Rad-Hydro ionization test problem: initialize grid (SUCCESS or FAIL) */
...
/* Rad-Hydro clump ionization problem: initialize grid (SUCCESS or FAIL) */
...
/* RH r^{-2} density ionization problem: initialize grid (SUCCESS or FAIL) */
...
/* Rad-Hydro test problem: cosmological HII ioniztion (SUCCESS or FAIL) */
...
#ifdef EMISSIVITY
/* Star Particle handler routine with emissivity. */
  int StarParticleHandler(int level, float dtLevelAbove);
#else
