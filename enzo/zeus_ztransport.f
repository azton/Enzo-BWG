...
c=======================================================================
c/////////////////////  SUBROUTINE ZEUS_ZTRANSPORT  \\\\\\\\\\\\\\\\\\\c
...
c  modified1:  Robert Harkness & Alexei Kritsuk
c              to ensure consistent minimum change factors in x, y and z
c  date:       June 2003
c
c  PURPOSE:
c
...
c=======================================================================
c
c     Compute variants on start indexes
c
      jsm1 = max(js-1, 1)
      jep1 = min(je+1, jn)
c
...
c
            ...
            enddo
            do k=ks-3, ke+2
c               uavgk(k) = 0.5*(w(i,j,k) + w(i,j,k+1))
...
c
     ...
     &                     dz, dt, uavgk, wstar)
c            call vanlr3_fc(ueff, 1, 1, kn, ks-1, ke+1, 1, 1,
c     &                     dz, dt, uavgk, wstar)
c
...
c
     ...
     &                       )
               ...
               if (e(i,j,k) .le. 0.0 .or. dnew .le. 0.0) then
                  ...
                  CALL f_error("zeus_ztransport.src",282)
               endif
...
c
         do n=0, nsubgrids-1
c        do j=fjstart(n*3+3)+1, fjend(n*3+3)+1
...
c            do i=fistart(n*3+3)+1, fiend(n*3+3)+1
