...
cut down on scheduling and thread creation overhead. In an effort to reduce scheduling,
we can also try static scheduling on this loop, and all others.
*/
...
#ifdef USE_GRACKLE
        startchem=MPI_Wtime();
          if ((grackle_data->use_grackle == TRUE) && RadiativeCooling) {
              /* Call Grackle */
          
            //fprintf(stderr, "%"ISYM": Calling GrackleWrapper\n", MyProcessorNumber);

            status[grid1] = Grids[grid1]->GridData->GrackleWrapper();
    
            //fprintf(stderr, "%"ISYM": Called GrackleWrapper\n", MyProcessorNumber);
            chemtime = MPI_Wtime() - startchem; 
          } // if grackle && radiative cooling
...
#pragma omp critical(othersubgrid)
	      while (Temp2 != NULL) 
        {
...
#endif
#ifdef TIMING
  //if ((LevelCycleCount[level] % 100 == 0) || (level==0)){
    float t_now = MPI_Wtime();
    float time_this_step = t_now-t_entry;
    fprintf(stdout, "TIMING %"ISYM" %"ISYM" iteration walltime = %"FSYM"\n", MyProcessorNumber, level, time_this_step);
    fprintf(stdout, "TIMING %"ISYM" %"ISYM" For number of grids = %"ISYM"\n", MyProcessorNumber, level, NumberOfGrids);
    MemInUse = mused();
    fprintf(stdout, "TIMING %"ISYM" %"ISYM" memory usage = %"ISYM"\n", MyProcessorNumber, level, MemInUse);
#ifdef USE_GRACKLE
    fprintf(stdout, "TIMING %"ISYM" %"ISYM" iteration chemistry time = %"FSYM"\n", MyProcessorNumber, level, chemtime);
#endif  //}
#endif
...
#ifdef MEM_TRACE
    MemInUse = mused();
    fprintf(memtracePtr, "  EL Level %"ISYM" to Level %"ISYM": n_grids %"ISYM" ; MEM %16"ISYM" \n", level, level+1, NumberOfGrids, MemInUse);
    fflush(memtracePtr);
#endif
