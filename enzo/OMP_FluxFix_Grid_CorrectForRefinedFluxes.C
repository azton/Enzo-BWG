...
/  mofified3:  Robert Harkness, March 2010
/              OpenMP & check for colour fields
/
...
#pragma omp parallel private (field, i, j, k, index) shared (Start, End, DensNum, Offset, RadiativeCooling, FieldDensityType) default (none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, RadiativeCooling, Start, End, GridDimension, DensNum, Offset, BaryonField, FieldDensityType) default(none)
#endif
  {
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private (field, i, j, k, index) shared (Start, End, DensNum, Offset) default (none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, Start, End, GridDimension, BaryonField, Offset, DensNum) default(none)
#endif
  {
#pragma omp for schedule(dynamic)
	  for (field = 0; field < NumberOfBaryonFields; field++) {
		...
		for (j = Start[1]; j <= End[1]; j++) {
		  ...
		  for (i = Start[0]; i <= End[0]; i++, index++) {
		    BaryonField[field][index] /= BaryonField[DensNum][index];
		    BaryonField[field][index+Offset] /= BaryonField[DensNum][index+Offset];
		  } // i
	  ...
	  for (field = 0; field < NumberOfBaryonFields; field++) {
	    ...
	    if (FieldTypeIsDensity(FieldType[field]) == TRUE ||
	      ...
	      FieldType[field] == InternalEnergy) CheckPositive = 1;
...
//			BaryonField[field][FieldIndex] -=
			LeftDelta = -
		      ...
		      if ( CheckPositive && (BaryonField[field][FieldIndex] + LeftDelta) <= 0) {
			//  New density-type field correction would result in field < 0
			//  This sets the effective deltas to zero for EVERY field.
			...
			BaryonField[field][FieldIndex] += LeftDelta;
		      }
...
//			BaryonField[field][FieldIndex + Offset] +=
			RightDelta =
		      ...
		      if ( CheckPositive && (BaryonField[field][FieldIndex + Offset] - RightDelta) <= 0.0) {
			//  New density-type field correction would result in field < 0
			...
			BaryonField[field][FieldIndex + Offset] += RightDelta;
		      }
...
#pragma omp parallel private (field, i, j, k, index) shared (Start, End, DensNum, Offset, RadiativeCooling, FieldDensityType) default (none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, RadiativeCooling, Start, End, GridDimension, BaryonField, Offset, DensNum, FieldDensityType) default(none)
#endif
  {
#pragma omp for schedule(dynamic)
	    for (field = 0; field < NumberOfBaryonFields; field++) {
...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private (i,j,k,i1, i2) shared (Start, End, DensNum, Offset, GENum, TENum, Vel1Num, Vel2Num, Vel3Num, tiny_number) default (none)
#else

#pragma omp parallel private(i, j, k, i1, i2) shared(Start, End, GridDimension, Offset, BaryonField, tiny_number, GENum, TENum, Vel1Num, Vel2Num, Vel3Num, GridRank) default(none)
#endif
  {
#pragma omp for schedule(dynamic)
	    for (k = Start[2]; k <= End[2]; k++) {
		...
		for (i = Start[0]; i <= End[0]; i++, i1++, i2++) {
		  ...
		  BaryonField[GENum][i1] = max(BaryonField[GENum][i1], tiny_number);
		  BaryonField[GENum][i2] = max(BaryonField[GENum][i2], tiny_number);
...
#pragma omp parallel private (field, i, j, k, index) shared (Start, End, DensNum, Offset) default (none)
#else
#pragma omp parallel private(field, i, j, k, index) shared(NumberOfBaryonFields, FieldType, Start, End, GridDimension, BaryonField, DensNum, Offset) default(none)
#endif
  {
#pragma omp for schedule(dynamic)
	  for (field = 0; field < NumberOfBaryonFields; field++) {
