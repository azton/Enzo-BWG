...
c
         do i=1,ddim1
            shift1 = temp(i,1,1,2)/max(temp(i,1,1,1),1.0e-20)*coef1
...
c
         do j=1, ddim2
            do i=1, ddim1
               shift2 = temp(i,j,1,3)/
     &                  max(temp(i,j,1,1),1.0e-20)*coef2
               y = min(max((j - 0.5 + shift2), half), edge2)
               j1 = int(y + 0.5)
               dy = real(j1) + 0.5 - y
               shift1 = temp(i,j,1,2)/
     &                  max(temp(i,j,1,1),1.0e-20)*coef1
               x = min(max((i - 0.5 + shift1), half), edge1)
               i1 = int(x + 0.5)
               dx = real(i1) + 0.5 - x
               mass = temp(i,j,1,1)
...
c
         do k=1, ddim3
            do j=1, ddim2
               do i=1, ddim1
                  shift3 = temp(i,j,k,4)/
     &                     max(temp(i,j,k,1),1.0e-20)*coef3
                  z = min(max((k - 0.5 + shift3), half), edge3)
                  k1 = int(z + 0.5)
                  dz = real(k1) + 0.5 - z
c
                  shift2 = temp(i,j,k,3)/
     &                     max(temp(i,j,k,1),1.0e-20)*coef2
                  y = min(max((j - 0.5 + shift2), half), edge2)
                  j1 = int(y + 0.5)
                  dy = real(j1) + 0.5 - y
c
                  shift1 = temp(i,j,k,2)/
     &                     max(temp(i,j,k,1),1.0e-20)*coef1
                  x = min(max((i - 0.5 + shift1), half), edge1)
                  i1 = int(x + 0.5)
                  dx = real(i1) + 0.5 - x
c
