...
#else
#pragma omp parallel private(grid1, grid2) shared(level, NumberOfGrids, Grids, MetaData, Exterior, status1, status2) default(none) if(level > 0)
#endif
  {
...
#else
#pragma omp parallel private(grid1, grid2) shared(level, NumberOfGrids, Grids, MetaData, status1, status2) default(none) if(level > 0)
#endif 
  {
#pragma omp for schedule(dynamic)
