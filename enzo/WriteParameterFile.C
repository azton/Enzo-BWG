...
int WriteParameterFile(FILE *fptr, TopGridData &MetaData)
{
  ...
  fprintf(fptr, "h2_on_dust                  = %d\n", grackle_data->h2_on_dust);
  fprintf(fptr, "three_body_rate             = %d\n", grackle_data->three_body_rate);
  fprintf(fptr, "cie_cooling                 = %d\n", grackle_data->cie_cooling);
  fprintf(fptr, "h2_optical_depth_approximation = %d\n", grackle_data->h2_optical_depth_approximation);
  fprintf(fptr, "photoelectric_heating       = %d\n", grackle_data->photoelectric_heating);
  fprintf(fptr, "photoelectric_heating_rate  = %lf\n", grackle_data->photoelectric_heating_rate);
  fprintf(fptr, "NumberOfDustTemperatureBins = %d\n", grackle_data->NumberOfDustTemperatureBins);
  fprintf(fptr, "DustTemperatureStart        = %lf\n", grackle_data->DustTemperatureStart);
  fprintf(fptr, "DustTemperatureEnd          = %lf\n", grackle_data->DustTemperatureEnd);
  fprintf(fptr, "SolarMetalFractionByMass    = %lf\n", grackle_data->SolarMetalFractionByMass);
  /********************************/
#endif
  ...
  fprintf(fptr, "PackedAMR                       = %"ISYM"\n", PackedAMR);

  fprintf(fptr, "LoadBalance0                    = %"ISYM"\n", LoadBalance0);
  fprintf(fptr, "LoadBalance1                    = %"ISYM"\n", LoadBalance1);
  fprintf(fptr, "LoadBalance2                    = %"ISYM"\n", LoadBalance2);
  ...
  fprintf(fptr, "StaticChainingMeshDimension     = %"ISYM"\n", StaticChainingMeshDimension);
  fprintf(fptr, "DynamicChainingMeshDimension    = %"ISYM"\n", DynamicChainingMeshDimension);
...
/* mechanical feedback parameters */
  ...
  float OutRho = SmallRho*DensityUnits;
  fprintf(fptr, "SmallRho                   = %"FSYM"\n", OutRho);
  ...
  time_t ID;
  ID = time(NULL);
  fprintf(fptr, "CurrentTimeIdentifier = %d\n", int(ID));
