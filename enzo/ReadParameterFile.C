...
int ReadParameterFile(FILE *fptr, TopGridData &MetaData, float *Initialdt)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
	...
	printf("Cannot set RebuildHierarchyCycleSkip[%"ISYM"], max hierarchy depth = %"ISYM".\n", int_dummy, MAX_DEPTH_OF_HIERARCHY);
    return FAIL;
      }
    ...
    ret += sscanf(line, "RadiationHydrodynamics = %"ISYM,
    ...
    // Extra Grackle parameters with no Enzo analog for this old Enzo.
    ret += sscanf(line, "h2_on_dust = %d", &grackle_data->h2_on_dust);
    ret += sscanf(line, "three_body_rate = %d", &grackle_data->three_body_rate);
    ret += sscanf(line, "cie_cooling = %d", &grackle_data->cie_cooling);
    ret += sscanf(line, "h2_optical_depth_approximation = %d", &grackle_data->h2_optical_depth_approximation);
    ret += sscanf(line, "photoelectric_heating = %d", &grackle_data->photoelectric_heating);
    ret += sscanf(line, "photoelectric_heating_rate = %lf", &grackle_data->photoelectric_heating_rate);
    ret += sscanf(line, "NumberOfDustTemperatureBins = %d", &grackle_data->NumberOfDustTemperatureBins);
    ret += sscanf(line, "DustTemperatureStart = %lf", &grackle_data->DustTemperatureStart);
    ret += sscanf(line, "DustTemperatureEnd = %lf", &grackle_data->DustTemperatureEnd);
    ret += sscanf(line, "SolarMetalFractionByMass = %lf", &grackle_data->SolarMetalFractionByMass);
    /********************************/
#endif

    ret += sscanf(line, "RadiativeCooling = %"ISYM, &RadiativeCooling);
    ret += sscanf(line, "RadiativeTransfer = %"ISYM, &RadiativeTransfer);
    ret += sscanf(line, "RadiativeTransferHydrogenOnly = %"ISYM, &RadiativeTransferHydrogenOnly);
    ...
    ret += sscanf(line, "PackedAMR = %"ISYM, &PackedAMR);

    ret += sscanf(line, "LoadBalance0 = %"ISYM, &LoadBalance0);

    ret += sscanf(line, "LoadBalance1 = %"ISYM, &LoadBalance1);

    ret += sscanf(line, "LoadBalance2 = %"ISYM, &LoadBalance2);
    ...
    ret += sscanf(line, "StaticChainingMeshDimension = %"ISYM, &StaticChainingMeshDimension);

    ret += sscanf(line, "DynamicChainingMeshDimension = %"ISYM, &DynamicChainingMeshDimension);
    ...
    ret += sscanf(line, "StarFeedbackKineticFraction = %"FSYM, &StarFeedbackKineticFraction);
    ret += sscanf(line, "StarMakerExplosionDelayTime = %"FSYM, &StarMakerExplosionDelayTime);
...
#ifdef RAD_HYDRO
  /* If RadiationHydrodynamics is on, ensure that RadiationFieldType is set
     to 0 so that the other Enzo routines will leave out all radiation physics,
     since those are taken care of in the coupled solver. */
  if ((RadiationHydrodynamics > 0) && (RadiationFieldType != 0)) {
    if (debug)
      fprintf(stderr, "WARNING: RadiationHydrodynamics is on. Forcing RadiationFieldType = 0\n");
    RadiationFieldType = 0;
  }

  /* If RadiationHydrodynamics is on, initialize rate data if it is not
     handled elsewhere */
  if ((RadiationHydrodynamics > 0) && (MultiSpecies == 0))
  ...
  /* Initialze Cosmic Ray Efficiency Models */
  if(CRModel)
    if(InitializeCosmicRayData() == FAIL){
      fprintf(stderr, "Error in Initialize CosmicRayData.\n");
      return FAIL;
    }
  ...
  if (grackle_data->use_grackle == TRUE) {
    ...
    if (initialize_chemistry_data(&grackle_units) == FAIL) {
      fprintf(stderr, "Error in Grackle initialize_chemistry_data.\n");
      return FAIL;
    }
  }  // if (grackle_data->use_grackle == TRUE)
  ...
  if(GadgetEquilibriumCooling == TRUE){
    ...
    if(MyProcessorNumber == ROOT_PROCESSOR ) {
      ...
      fprintf(stderr, "WARNING:  RadiationFieldType = 0, MultiSpecies = 0, MetalCooling = 0, and\n");
...
//  runs unless you know what you're doing.  (BWO)
  ...
  if (TracerParticleOn || StarParticleCreation) {
