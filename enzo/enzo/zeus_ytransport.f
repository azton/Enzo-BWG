...
c=======================================================================
c/////////////////////  SUBROUTINE ZEUS_YTRANSPORT  \\\\\\\\\\\\\\\\\\\c
...
c
            do j=js-3, je+2
c               uavgj(j) = 0.5*(v(i,j,k) + v(i,j+1,k))
...
c
     ...
     &                     dy, dt, uavgj, vstar)
c            call vanlr2_fc(ueff, 1, jn, 1, js-1, je+1, 1, 1,
c     &                     dy, dt, uavgj, vstar)
            if (rank .gt. 2) 
...
c
     ...
     &                       )
               ...
               if (e(i,j,k) .le. 0.0 .or. dnew .le. 0.0) then
                  ...
                  CALL f_error("zeus_ytransport.src",280)
               endif
...
c
         do n=0, nsubgrids-1
c        do i=fistart(n*3+2)+1, fiend(n*3+2)+1
...
c            do k=fjstart(n*3+2)+1, fjend(n*3+2)+1
