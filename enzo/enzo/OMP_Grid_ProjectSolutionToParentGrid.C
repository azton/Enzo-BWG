...
#pragma omp for schedule(dynamic)
  ...
  if (ProcessorNumber == MyProcessorNumber) {
    ...
    float weight;
// below in INTEL_OMP_SYNTAX had shared "Velocity1, Velocity2, Velocity3,", but these are predetermined as shared? so cant be included in the pragma...
#ifdef INTEL_OMP_SYNTAX
#pragma omp parallel private(field, i, j, k, i1, j1, k1, skipi, skipj, skipk, weight, pindex, gindex) shared(Refinement, HydroMethod, ParentStartIndex, Dim, ParentGrid, ParentDim, RelativeVolume) default(none)
#else
