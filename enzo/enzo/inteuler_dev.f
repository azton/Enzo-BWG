...
c
      integer*8 gravity, i1, i2, idim, idual, iflatten, ipresfree, 
     &        isteep, j1, j2, jdim, ncolor, iconsrec, iposrec
      real*8  dt, eta1, eta2, gamma
      real*8  dslice(idim,jdim),     dxi(idim     ),  pslice(idim,jdim),
     &        uslice(idim,jdim),  vslice(idim,jdim),  wslice(idim,jdim),
     &        grslice(idim,jdim), geslice(idim,jdim)
      real*8  dls(idim,jdim),     drs(idim,jdim), flatten(idim,jdim),
     &        pls(idim,jdim),
     ...
     &        vrs(idim,jdim),     wls(idim,jdim),     wrs(idim,jdim)
      real*8  colslice(idim,jdim,ncolor),   colls(idim,jdim,ncolor),
     &        colrs(idim,jdim,ncolor)
c
c  local declarations
c
      integer*8 i, j, n, m, ic
      real*8 steepen(ijkn),tmp1(ijkn),tmp2(ijkn),tmp3(ijkn),tmp4(ijkn)
      real*8 qa,qb,qc,qd,qe,s1,s2,f1
      real*8  c1(ijkn), c2(ijkn), c3(ijkn), c4(ijkn), c5(ijkn), c6(ijkn)
     &    ,c7(ijkn), c8(ijkn)
     ...
     &    ,colla(ijkn,35),colra(ijkn,35)
     &    ,coll0(ijkn,35),colr0(ijkn,35)
     &    ,dcol(ijkn,35),coll(ijkn,35)
     &    ,colr(ijkn,35),col6(ijkn,35)
      real*8 lem(idim,5,5),rem(idim,5,5), dxdt2(ijkn)
c
c  parameters
c
      real*8 ft
...
c
     ...
     &           /((dxb(i) + dxb(i-1))*
     &           (dslice(i+1,j) - dslice(i-1,j) + 1.d-35))
...
c
      do i=i1, i2+1
         pls(i,j) = max(pls(i,j), 1.d-35)
         prs(i,j) = max(prs(i,j), 1.d-35)
         dls(i,j) = max(dls(i,j), 1.d-35)
         drs(i,j) = max(drs(i,j), 1.d-35)
      enddo
      do ic = 1,ncolor
         do i = i1,i2+1
            colls(i,j,ic) = max(colls(i,j,ic), 1.d-35)
            colrs(i,j,ic) = max(colrs(i,j,ic), 1.d-35)
         enddo
      enddo
c
