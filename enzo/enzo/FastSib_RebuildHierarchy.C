		     ...
		     LevelHierarchyEntry *LevelArray[], int level)
{
  ...
  printf("LevelSubCycleCount = %"ISYM"; CycleSkip on this level = %"ISYM"\n",LevelSubCycleCount[level], 
          RebuildHierarchyCycleSkip);
  if (LevelSubCycleCount[level] % RebuildHierarchyCycleSkip) {
    if (debug) printf("REBUILD SKIPPED\n");
    return SUCCESS;
  }
  int dbx = 0;
...
//??      HierarchyEntry *GridHierarchyPointer[MAX_NUMBER_OF_SUBGRIDS];
	...
	Temp                                = Temp->NextGridThisLevel;
      ...
      /* 3e) For each new subgrid, interpolate from parent and then copy
