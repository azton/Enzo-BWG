...
c
      ...
      parameter (ijkn = 4103, num_iter = 2)
...
c
     ...
     &                 real(6*refine1**3                     )
         coef01(i+1) = 1.0/refine1 - coefm1(i+1) - coefp1(i+1)
      enddo
c
      do i = 0, refine2-1
     ...
     &                 real(6*refine2**3                     )
         coef02(i+1) = 1.0/refine2 - coefm2(i+1) - coefp2(i+1)
      enddo
c
      do i = 0, refine3-1
     ...
     &                 real(6*refine3**3                     )
         coef03(i+1) = 1.0/refine3 - coefm3(i+1) - coefp3(i+1)
      enddo
c
...
c    the same.
c
      dV = 1.0/real(refine1*refine2*refine3)
      do iter = 1, num_iter
c
...
c
     ...
     &                 grid((i-1)*refine1+gstart1  , jgrid, kgrid)
c
            if (abs(del2left) .lt. 1.0e-20) 
     &           del2left = sign(1.0e-20, del2left)
            fraction(i) = min(max(del1left/del2left, 0.0), 1.0)
         enddo
c
...
c
     ...
     &                  grid( i   *refine1+gstart1-1, jgrid, kgrid)
c
            if (abs(del2right) .lt. 1.0e-20) 
     &           del2right = sign(1.0e-20, del2right)
            fraction(i) =  max(max(min(del1right/del2right, 0.0), 1.0),
     &                         fraction(i)                            )
         enddo
c
...
c     &         grid(i-start1+gstart1, jgrid, kgrid)
c     &                        * (1.0 - fraction((i-start1)/refine1+1) )
c         enddo
c
         do i1 = gstart1, gstart1+refine1-1
            do i2 = 1, wdim1
     ...
     &                     fraction(i2)  * temp(i2) +
     &              (1.0 - fraction(i2)) * grid((i2-1)*refine1+i1, 
     &                                          jgrid, kgrid)
            enddo
         enddo
c
...
c
            if (abs(del2left) .lt. 1.0e-20) 
     &           del2left = sign(1.0e-20, del2left)
            fraction(j) = min(max(del1left/del2left, 0.0), 1.0)
         enddo
c
c        repeat for right side of each distribution
c
...
c
            if (abs(del2right) .lt. 1.0e-20) 
     &           del2right = sign(1.0e-20, del2right)
            fraction(j) =  max(max(min(del1right/del2right, 0.0), 1.0),
     &                         fraction(j)                            )
         enddo
c
c        Modify distributions based on the computed fraction decrease
...
c
            if (abs(del2left) .lt. 1.0e-20) 
     &           del2left = sign(1.0e-20, del2left)
            fraction(k) = min(max(del1left/del2left, 0.0), 1.0)
         enddo
c
c        repeat for right side of each distribution
c
...
c
            if (abs(del2right) .lt. 1.0e-20) 
     &           del2right = sign(1.0e-20, del2right)
            fraction(k) =  max(max(min(del1right/del2right, 0.0), 1.0),
     &                         fraction(k)                            )
         enddo
c
c        Modify distributions based on the computed fraction decrease
c
     ...
     &                     fraction(k2)  * temp(k2) +
     &              (1.0 - fraction(k2)) * grid(igrid, jgrid,
     &                                         (k2-1)*refine3+k1)
            enddo
         enddo
c
