...
c
     ...
     &        ihydro
      real*8 dx(idim), dy(jdim), dz(kdim)
...
c
      if (rank .eq. 1) then
         do i = i1+1, i2+1
            cs = max(sqrt(gamma*p(i,1,1)/d(i,1,1)), 1.0e-20)
            if (ipfree .eq. 1) cs = 1.0e-20
            dt = min(dt, real(dx(i)*aye/(cs + abs(u(i,1,1)-vgx))))
         enddo
         if (ihydro .eq. 2) then
            do i = i1+1, i2+1
               dtviscous = min(dtviscous,
     &     real(dx(i)*aye/(4.0*C2*max(-u(i+1,1,1)+u(i,1,1),1.0e-20))))
            enddo
         endif
      endif
c
...
c
      if (rank .eq. 2) then
         do j = j1+1, j2+1
            do i = i1+1, i2+1
               cs = max(sqrt(gamma*p(i,j,1)/d(i,j,1)), 1.0e-20)
               if (ipfree .eq. 1) cs = 1.0e-20
               if (ihydro .eq. 0) then
c
...
c
            if (ihydro .eq. 2) then
               do i = i1+1, i2+1
                  dtviscous = min(dtviscous, 
     &      real(dx(i)*aye/(4.0*C2*max(-u(i+1,j,1)+u(i,j,1),1.0e-20))),
     &      real(dy(j)*aye/(4.0*C2*max(-v(i,j+1,1)+v(i,j,1),1.0e-20))))
               enddo
            endif
         enddo
      endif
...
c
     ...
     &               write(6,*) 'calc_dt',d(i,j,k),p(i,j,k),i,j,k
                  cs = max(sqrt(gamma*p(i,j,k)/d(i,j,k)), 1.0e-20)
                  if (ipfree .eq. 1) cs = 1.0e-20
                  if (ihydro .eq. 0) then
c     
c     Godunov's formula.
...
c
               if (ihydro .eq. 2) then
                  do i = i1+1, i2+1
	            dt1 = min(
     &          dx(i)*aye/(4.0*C2*max(-u(i+1,j,k)+u(i,j,k),1.0e-20)),
     &          dy(j)*aye/(4.0*C2*max(-v(i,j+1,k)+v(i,j,k),1.0e-20)),
     &          dz(k)*aye/(4.0*C2*max(-w(i,j,k+1)+w(i,j,k),1.0e-20)))
