...
c	
      if (ii .ge. nmax) then
         ...
         CALL f_error("star_maker3mom.src",266)
      endif
      np = ii
c
c      if (np .ne. 0) then
...
c
     ...
     &              kin_energy_before)
     &              /(kin_energy_before)
               CALL f_error("star_maker3mom.src",837)
            endif
...
c
     ...
     &              mass_before)
     &              /(mass_before)
               CALL f_error("star_maker3mom.src",855)
            endif
...
c
      if (idir .ne. -1_IKIND .and. idir .ne. 1_IKIND) then
         ...
         CALL f_error("star_maker3mom.src",930)
      endif
c
...
c
      if (imulti_metals .eq. 1_IKIND) then
         ...
         CALL f_error("star_maker3mom.src",1160)
      endif
c
