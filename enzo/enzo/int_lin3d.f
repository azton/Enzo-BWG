...
c
               if (delf0 .eq. 0.0) 
     &              delf0 = sign(1.0e-20, delf0 + delf1 + delf2)
...
c
     ...
     &                                    + (1.0-chi2)*delf2
     &                                    + (1.0-chi3)*delf3 ) /
     &           (chi1*delf1 + chi2*delf2 + chi3*delf3 + 1.0e-20)
c
               frac = min(frac, 1.0)
c
...
c
     ...
     &                          gstart3+(k-1)*refine3+k1) .le. 0.0) then
               ...
               CALL f_error("int_lin3d.src",224)
                        endif
                     enddo
                  enddo
               enddo
