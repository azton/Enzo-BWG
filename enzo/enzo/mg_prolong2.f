...
c
      fact1 = real(sdim1)/real(ddim1)
      fact2 = real(sdim2)/real(ddim2)
      fact3 = real(sdim3)/real(ddim3)
...
c
      if (ndim .eq. 1) then
         do i=1, ddim1
            x = min(max((real(i)-0.5)*fact1, half), edge1)
            i1 = int(x + 0.5)
            dx = real(i1) + 0.5 - x
...
c
      if (ndim .eq. 2) then
         do j=1, ddim2
            y = min(max((real(j)-0.5)*fact2, half), edge2)
            j1 = int(y + 0.5)
            dy = real(j1) + 0.5 - y
            do i=1, ddim1
               x = min(max((real(i)-0.5)*fact1, half), edge1)
...
c
      if (ndim .eq. 3) then
         do k=1, ddim3
            z = min(max((real(k)-0.5)*fact3, half), edge3)
            k1 = int(z + 0.5)
            dz = real(k1) + 0.5 - z
            do j=1, ddim2
               y = min(max((real(j)-0.5)*fact2, half), edge2)
