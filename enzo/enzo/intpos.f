...
c
      ...
      real gamma
      real dslice(idim), uslice(idim), pslice(idim), dxdt2(idim),
...
c     
      do i = i1, i1+1
         if (drho(i) .gt. 0.5*dslice(i)) then
            drho(i) = 0.5*dslice(i)
            dla(i) = dslice(i) - 0.5*drho(i)
            dra(i) = dslice(i) + 0.5*drho(i)
            dl0(i) = dla(i)
            dr0(i) = dra(i)
            print*, 'Enforcing positive pressure by limiting ' ,
     $           'density slope', i, dslice(i), drho(i)
            CALL f_warning("intpos.src",78)
         endif
      enddo
c
c     Velocity1
c     
      gamma1i = 1.0 / (gamma + 1.0)
      do i = i1, i1+1
         limit = 2.0 * gamma1i * dxdt2(i)
         if (du(i) .gt. limit) then
            du(i) = limit
            ula(i) = uslice(i) - 0.5*du(i)
            ura(i) = uslice(i) + 0.5*du(i)
            ul0(i) = ula(i)
            ur0(i) = ura(i)
            print*, 'Enforcing positive pressure by limiting ' ,
     $           'velocity slope', i, uslice(i), du(i)
            CALL f_warning("intpos.src",95)
         endif
      enddo
c
c     Pressure
c     
      do i = i1, i1+1
         ...
         if (du(i) .gt. limit) then
            dp(i) = limit
            pla(i) = pslice(i) - 0.5*dp(i)
            pra(i) = pslice(i) + 0.5*dp(i)
            pl0(i) = pla(i)
            pr0(i) = pra(i)
            print*, 'Enforcing positive pressure by limiting ' ,
     $           'pressure slope', i, pslice(i), dp(i)
            CALL f_warning("intpos.src",111)
         endif
      enddo
